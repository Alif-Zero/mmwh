            ...
            if not self.invoice_picking_id:
                ...
                if self.picking_type_id.code == 'incoming':
                    pick = {
                        ...
                        'partner_id': self.partner_id.id,
                        'origin': self.name,
                        'location_dest_id':self.location_id.id or  self.picking_type_id.default_location_dest_id.id,
                ...
                if self.picking_type_id.code == 'incoming':
                    ...
                    move_ids.move_line_ids.write({'lot_name': order.name})
                    move_ids._action_assign()
                if self.picking_type_id.code == 'outgoing':
                    move_ids = moves.create_move_line_product(picking, self.invoice_id)
                    move_ids = moves._action_confirm()
                    move_ids._action_assign()
...
class StockMove(models.Model):
    _inherit = 'stock.move'
    def create_move_line_product(self, picking, invoice):
        for line in self:
            price_unit = line.price_unit
            ...
            self.env['stock.move.line'].create(template)